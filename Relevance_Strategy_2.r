...
mkt_ret              = list()
w_len                = 28
mean_weights         = c(w_len:1/sum(1:w_len)) 
for(j in (which(dat$date == "2018-01-01"):(nrow(dat)))){
	...
	most_relevant           = macro_relevance_ordered[1:w_len] 
	...
	## Compute moments for optimal portfolio weights 
        ...
        sd_ret    = asset_ret %>% sapply(., function(x) (weighted.var(x, w = mean_weights))^0.5) 
	sr_ret    = exp_ret/sd_ret

	## Compute portfolio weights proportionally to 
	## the size of the expected return
	opt_wgt   = sr_ret/sum(sr_ret)
...
print(alpha_test)
ret_test       = 
	summary(lm(unlist(strategy_ret)-unlist(mkt_ret) ~ 1))
print(ret_test)
